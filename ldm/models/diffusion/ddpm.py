...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        if self.model.conditioning_key is not None and not self.force_null_conditioning:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ["caption", "coordinates_bbox", "txt", "tks"]:
                    xc = batch[cond_key]
    ...
    def get_input_foralpha4(
        ...
        trans = transforms.RandomAffine(degrees=0, translate=(0.008, 0.008))
        x = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
        ...
        for i in range(x_aug.shape[0]):
            x_aug[i : i + 1] = trans(x[i : i + 1])

        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()

        encoder_posterior_aug = self.encode_first_stage(x_aug)
        z_aug = self.get_first_stage_encoding(encoder_posterior_aug).detach()
...
class PLACE_LDM(LatentDiffusion):
    def __init__(self, control_key, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.control_key = control_key
        self.colors = colormap()  # loadmat('color150.mat')['colors']
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        viewcontrol = batch["viewcontrol"]
        tkscls = batch["tokens_cls"]
        if bs is not None:
            control = control[:bs]
            viewcontrol = viewcontrol[:bs]
            tkscls = tkscls[:bs]

        control = control.to(self.device)
        viewcontrol = viewcontrol.to(self.device)
        tkscls = tkscls.to(self.device)
        control = einops.rearrange(control, "b h w c -> b c h w")
        viewcontrol = einops.rearrange(viewcontrol, "b h w c -> b c h w")

        control = control.to(memory_format=torch.contiguous_format).float()
        viewcontrol = viewcontrol.to(memory_format=torch.contiguous_format).float()
        tkscls = tkscls.to(memory_format=torch.contiguous_format).float()
        return x, dict(
            c_crossattn=[c],
            c_concat=[control],
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        cond_tkscls = torch.cat(cond["tkscls"], 1)
    ...
    def log_images(
        ...
        **kwargs,
        ...
        view_ctrol = view_ctrol * 255.0 - 1
        view_ctrol_np = view_ctrol.detach().cpu().permute((0, 2, 3, 1)).numpy()
        newview = np.zeros((view_ctrol.shape[0], 512, 512, 3))
        for idx in range(view_ctrol.shape[0]):
            newview[idx] = colorEncode(view_ctrol_np[idx, :, :, 0], self.colors)
        newview = torch.tensor(
            newview, dtype=view_ctrol.dtype, device=view_ctrol.device
        ).permute((0, 3, 1, 2))
        newview = newview / 255.0

        log["control"] = newview * 2.0 - 1.0
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_cross[:, 0] = 49406
            uc_cross = self.cond_stage_model.encode(uc_cross.to(self.device))
    ...
    def configure_optimizers(self):
        ...
        opt1 = torch.optim.AdamW(params1, lr=lr)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        if self.training:
            # only compute losses during training
