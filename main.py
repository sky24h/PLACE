...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    print(
        f"Missing {len(m)} keys, unexpected {len(u)} keys, matched {len(sd)-len(m)-len(u)} keys."
    )
...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        "--data_root", type=str, required=True, help="Path to dataset directory"
    )
...
class SetupCallback(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):
        ...
        os.makedirs(os.path.join(self.logdir, "tblogger"), exist_ok=True)
...
class ImageLogger(Callback):
    def __init__(
        ...
        self.logger_log_images = {
            pl.loggers.TensorBoardLogger: self._tblogger,
        }
    ...
    @rank_zero_only
    def _tblogger(self, pl_module, images, batch_idx, split):
        for k in images:
...
if __name__ == "__main__":
    ...
    parser = get_parser()
    # "add_argparse_args" was removed after pytorch_lightning 2.0.
    # !TODO Update the code to be compatible with version 2.0+ in the future.
    parser = Trainer.add_argparse_args(parser)
    ...
    try:
        ...
        trainer_config["devices"] = opt.gpus
        ...
        default_logger_cfg = default_logger_cfgs["tblogger"]
